...
const EmailForm = ({ 
	...
	<Form {...form}>
		<form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 min-h-[110px] ">
			<FormField
...
export default function MagicLink({
	...
	if (isSent) {
		return <SuccessState email={form.getValues('email')} onReset={handleReset} />;
	}

	return (
		<div className={className}>
			<EmailForm form={form} isLoading={isLoading} onSubmit={onSubmit} />
		</div>
	);
}
